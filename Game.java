    ...
    public String checkGameWinner(char [][]grid){
       ...
       //diagnal
        ...
        for (int i=0; i<whoseturn.length; i++) {
            // check diagnols    
        	if (grid[0][0]==whoseturn[i]&& grid[1][1]==whoseturn[i]&& grid[2][2]==whoseturn[i]) {
                     return message[i];
                }
                if (grid[0][2]==whoseturn[i] && grid[1][1]==whoseturn[i]
                        && grid[2][0]==whoseturn[i]) {
                  return message[i];
                } 
           	
     		// Checks rows and columns. 
       for (int a=0; a<3; a++) {
    	   if(grid[a][0]==whoseturn[i]&&  grid[a][1]==whoseturn[i]&& grid[a][2]==whoseturn[i]) {
    		   return message[i]; 
    	   }
    	   if (grid[0][a]==whoseturn[i]&& grid[1][a]==whoseturn[i]&& grid[2][a]==whoseturn[i]){
    		   return message[i];
    	   }
       }
                }
